...
const INVESTOR_ID = "6805e8297f482b5677025898";

const SRC_PATH = "C:\\Users\\admin\\Downloads\\EGM Notice_16.09.2026.pdf";
const FILE_ID = "p1609202-6000-4000-8000-000000000001";
const FILE_NAME = "EGM Notice._16.09.2026.pdf";
const STORED_NAME = `${FILE_ID}.pdf`;
const RELATIVE_URL = `/uploads/investors/${INVESTOR_ID}/${STORED_NAME}`;
const MIME_TYPE = "application/pdf";
const TITLE = "EGM Notice._16.09.2026";
...
async function run() {
  try {
    ...
    fs.writeFileSync(path.join(uploadDir, "EGM Notice_16.09.2026.pdf"), pdfBuffer);
    ...
    fs.writeFileSync(path.join(directUploadsDir, "EGM Notice_16.09.2026.pdf"), pdfBuffer);
    ...
    if (!investorDoc) {
      console.log(`Investor document with ID ${INVESTOR_ID} (Preferential Issue) not found. Inserting new document.`);
      ...
      let updatedFiles = (investorDoc.files || []).filter(
        f => f.id !== FILE_ID && f.storedName !== STORED_NAME && f.title !== TITLE && f.originalName !== FILE_NAME && f.originalName !== "EGM Notice_16.09.2026.pdf"
      );
    ...
    if (fs.existsSync(jsonPath)) {
      ...
      const prefItem = data.find(e => e.id === INVESTOR_ID || e.title.toLowerCase().trim() === 'preferential issue');
      if (prefItem) {
        let files = (prefItem.files || []).filter(
          f => f.id !== FILE_ID && f.storedName !== STORED_NAME && f.title !== TITLE && f.originalName !== FILE_NAME && f.originalName !== "EGM Notice_16.09.2026.pdf"
        );
        // Place at the very top (index 0)
        files.unshift(fileEntry);
        prefItem.files = files;
        prefItem.updatedAt = new Date().toISOString();
        fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2), 'utf8');
        console.log("Updated data/admin-investors.json successfully.");
      } else {
        console.warn("Preferential Issue entry not found in admin-investors.json");
      }
    }
